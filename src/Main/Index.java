...
public class Index extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		EnterSuccessResult = 1; // 초기화
		  ...
		  List<Chat> nearChat = new ArrayList<Chat>(); 
		  nearChat = findNearEvent(ip); 
		  if(nearChat != null) {
			  request.setAttribute("nearChats", nearChat);
		  }
		  for(Chat chat: nearChat) {
			  System.out.println("근처의 챗 타이틀: " + chat.getChat_title());
		  }
	...
	public List<Chat> findNearEvent(String ip) {
		// ip = "192.168.0.21";
		List<Chat> nearEvent = new ArrayList<Chat>();
		List<Integer> nearEventCode = new ArrayList<Integer>(); // result_chatcode
		...
		ChatDAO chatDao = ChatDAO.getInstance();
		nearEventCode = questionDAO.getAllChatCode(buff.toString());
		if (nearEventCode != null) {
			for (Integer code : nearEventCode) {
				try {
					nearEvent.add(chatDao.getChat(Integer.toString(code)));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
