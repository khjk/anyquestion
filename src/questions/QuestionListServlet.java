...
public class QuestionListServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(listType == null || listType.equals("")) {
			...
			response.getWriter().write(getTen(chatCode));
		}else if(chatCode !=null){
			try {
				//가장 마지막으로 받은 q_no
				Integer.parseInt(listType);
				//q_no을 받았다면 넣음.
				response.getWriter().write(getListByID(listType, chatCode));
			} catch (Exception e) {
				response.getWriter().write("");
			}
		}
	...
	// 질문 불러옴
	public String getListByID(String questionNo, String cCode) {
		StringBuffer result = new StringBuffer("");
		result.append("{\"result\":[");
		QuestionDAO questionDAO = new QuestionDAO();
		if(questionNo !=null && cCode !=null) {
			ArrayList<Question> questionList = questionDAO.getQuestionListByRecent(questionNo, cCode);
	...
	public String getTen(String cCode) {
		StringBuffer result = new StringBuffer("");
		result.append("{\"result\":[");
		QuestionDAO questionDAO = new QuestionDAO();
		ArrayList<Question> questionList = questionDAO.getQuestionListByRecent(10, cCode);
		if(questionList.size() == 0 ) return "";
		for(int i = 0; i < questionList.size(); i++) {
			result.append("[{\"value\": \"" + questionList.get(i).getQuestionContent()+ "\"},");
			result.append("{\"value\": \"" + questionList.get(i).getLikeCount()+ "\"},");
